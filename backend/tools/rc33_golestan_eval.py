"""Run the fixed 20-question Golestan extraction regression through live RAG."""
...
_QUESTIONS = [
    ...
    ("q16", "درخواست انتقالی چگونه ثبت می‌شود؟", "سامانه سجاد"),
...
def main() -> int:
    """Run retrieval and generation and persist aggregate + per-question rows."""
    ...
    with httpx.Client(timeout=240.0) as client:
        for question_id, question, expected in _QUESTIONS:
            ...
            retrieval = client.post(
                f"{_BASE}/workspaces/{_WORKSPACE_ID}/retrieve",
                headers=_HEADERS,
                json={
                    "query": question,
            ...
            chat = client.post(
                f"{_BASE}/workspaces/{_WORKSPACE_ID}/chat",
                headers=_HEADERS,
                json={
                    "question": question,
                    "knowledge_base_id": args.knowledge_base_id,
    ...
    summary = {
        ...
        "pass_rate": _mean(row["verdict"] == "pass" for row in rows),
